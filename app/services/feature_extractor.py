...
"""
Feature Extractor for ML Confidence Prediction - FIXED
"""
import logging
...
@dataclass
class GenomicFeatures:
    """Container for extracted genomic features - FIXED"""
    chromosome: str
    position: int
    chain_score: float
    chain_count: int
    ...
    sv_overlap: bool
    segdup_overlap: bool
    assembly_gap_distance: int
    historical_success_rate: float
    cross_reference_agreement: float
    build_from: str
    build_to: str
    
    def to_array(self) -> np.ndarray:
        """Convert to numpy array for ML - FIXED dimensions"""
        return np.array([
            float(self.chain_score),
            float(self.chain_count),
            float(self.chain_gap_size),
            float(self.gc_content),
            float(self.repeat_density),
            float(self.low_complexity),
            float(self.sv_overlap),
            float(self.segdup_overlap),
            float(self.assembly_gap_distance),
            float(self.historical_success_rate),
            float(self.cross_reference_agreement)
        ], dtype=np.float32)


class FeatureExtractor:
    """Extract genomic features for ML - FIXED implementation"""
    
    def __init__(self, data_dir: str = None):
        self.data_dir = data_dir or settings.REFERENCE_DIR
        self.repeat_cache = {}
        self.sv_cache = {}
        self.historical_stats = {}
    ...
    def extract_features(
        ...
        """Extract all features - FIXED to return correct object"""
        
        # Chain file features
        chain_score = float(chain_result.get("confidence", 1.0)) if chain_result else 0.0
        chain_count = 1 if chain_result and chain_result.get("success") else 0
        chain_gap_size = 0
        
        # Sequence context (use actual values in production)
        gc_content = 0.41  # Genome-wide average
        repeat_density = 0.0
        repeat_type = "none"
        ...
        # Structural features
        sv_overlap = False
        segdup_overlap = False
        gap_distance = 1000000
        
        # Historical features
        historical_success = 0.95
        cross_ref_agreement = 0.90
